...
static const char *TAG = "lvgl_2048";
...
static void user_task(void *pvParameter)
{
    ...
    lv_label_set_text(label, "no datasssss");
#endif
    while (1)
    {
        ...
        input_device.read(&touchpad_data);
        if (touchpad_data.state == LV_INDEV_STATE_REL) {
            ...
            if(fabs(x_diff) > SENSITIVE || fabs(y_diff) > SENSITIVE) {
                if (fabs(x_diff) > fabs(y_diff)) {
                    if (x_diff > 0) {
                        move_right();
#ifdef DEBUG
                        lv_label_set_text(label, "move_right");
#endif
                    } else {
                        move_left();
#ifdef DEBUG
                        lv_label_set_text(label, "move_left");
#endif
                    }
                } else {
                    if (y_diff > 0) {
                        move_down();
#ifdef DEBUG
                        lv_label_set_text(label, "move_down");
#endif
                    } else {
                        move_up();
#ifdef DEBUG
                        lv_label_set_text(label, "move_up");
#endif
                    }
                }
            }
