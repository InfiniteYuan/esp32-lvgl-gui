...
// Commands list copied from https://github.com/adafruit/Adafruit-NT35510-Library
#define DELAY 0x80
...
LLDSPEC void gdisp_lld_fill_area(GDisplay *g) 
{
    ...
    acquire_bus(g);
    set_viewport (g);
    write_data_repeats (g,g->p.color,g->p.cx, g->p.cy);
    release_bus(g);
}
...
LLDSPEC void gdisp_lld_control(GDisplay *g) 
{
    ...
    case GDISP_CONTROL_ORIENTATION:
        ...
        switch((orientation_t)g->p.ptr) {
            case GDISP_ROTATE_0:
                acquire_bus(g);
                write_cmd(g, NT35510_MADCTL);
                write_data_byte(g, 0xC0|NT35510_MADCTRL_COLOR);
                g->g.Height = GDISP_SCREEN_HEIGHT;
                g->g.Width = GDISP_SCREEN_WIDTH;
                release_bus(g);
                break;
            case GDISP_ROTATE_90:
                acquire_bus(g);
                write_cmd(g, NT35510_MADCTL);
                write_data_byte(g, 0xA0|NT35510_MADCTRL_COLOR);
            ...
            case GDISP_ROTATE_180:
                acquire_bus(g);
                write_cmd(g, NT35510_MADCTL);
                write_data_byte(g, 0x00|NT35510_MADCTRL_COLOR);
                g->g.Height = GDISP_SCREEN_HEIGHT;
                g->g.Width = GDISP_SCREEN_WIDTH;
                release_bus(g);
                break;
            case GDISP_ROTATE_270:
                acquire_bus(g);
                write_cmd(g, NT35510_MADCTL);
                write_data_byte(g, 0x60|NT35510_MADCTRL_COLOR);
                g->g.Height = GDISP_SCREEN_WIDTH;
                g->g.Width = GDISP_SCREEN_HEIGHT;
                release_bus(g);
                break;
